...
def build_providers(
    ...
    In ``auto`` mode, providers that fail to initialize (for example
    ``akshare`` is not installed in the running Python) are skipped with a
    ``logger.warning`` so the caller can still use the remaining providers
    and the user is told why the chain is degraded. When the caller asks
    for a specific provider explicitly, init failures still raise so the
    silent-degradation footgun does not bite.

    See :func:`build_providers_full` when you also need the warning messages
    to surface them in a structured result.
    """
